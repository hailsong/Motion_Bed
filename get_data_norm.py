...
if __name__ == "__main__":
    ...
    print('../working/' + str(args.target))
    print(str(args.target))
    cap = cv2.VideoCapture('../working/' + str(args.target))
    ...
    while cap.isOpened():
        ...
        if not success:
            ...
            '''
            experiment_df.to_csv('../working/' + new_name + new_num + '_63.csv')
        ...
        if results.multi_hand_landmarks:
            ...
            for hand_landmarks in results.multi_hand_landmarks: #hand_landmarks는 감지된 손의 갯수만큼의 원소 수를 가진 list 자료구조
                ...
                for i in range(21):
                    ...
                    mark_p.append(mark_pixel(xval, yval, zval)) # 마크 픽셀 클래스 인스턴스를 사용하여 mark_p 리스트에 넣음
                ...
                zmax = max(zvalue)
                for i in range(21):
                    mark_p[i].x = mark_p[i].x / xmax
                    mark_p[i].y = mark_p[i].y / ymax
                    mark_p[i].z = mark_p[i].z / zmax
        ...
        if cv2.waitKey(5) & 0xFF == 27:
            ...
            experiment_df.to_csv('../working/' + name + '.csv', encoding='utf-8-sig')
